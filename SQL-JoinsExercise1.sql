...
SELECT p.name AS Product_Name, sum(s.quantity) AS Total_Sold, s.priceperunit * SUM(s.Quantity) AS Total_Price_Sold
FROM bestbuy.products p
INNER JOIN bestbuy.Sales s 
...
SELECT e.EmployeeID, e.FirstName, e.LastName, p.Name, SUM(s.quantity) AS Total_Sold
